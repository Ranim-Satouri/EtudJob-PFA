...
const fetchCategories = async () => {
  ...
  const handlepostuler = async(offreId) => {
    ...
    try {
      ...
      console.log(userData) // Obtenez l'e-mail de l'utilisateur depuis les données utilisateur
      const response = await fetch(`http://localhost:8090/sendEmail?to=${userEmail}&subject=EtudJob&text=je suis intereses par votre offre`, {
        method: "GET",
        headers: {
      ...
      if (!response.ok) {
        ...
        console.log("postulation avec succes");
        setsucces("mail envoyer avec succe.");
      return setTimeout(() => {
        setsucces(""); 
      }, 2000);
      ...
      setErreur("probléme de postulation.");
      return setTimeout(() => {
        setErreur(""); 
      }, 2000);
    }
  ...
  const handleClick = async(offreId, userId) => {
    ...
    try {
      const response = await fetch(`http://localhost:8090/addsave/${userId}/${offreId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
          ...
          <div className={styles.box} >
            ...
            <div key={index} className={styles.offre}>
              <div className={styles.part} >
                <img src={Save} alt="" className={styles.saveb} onClick={() => handleClick(offre.idOffre, idUser)} />
                ...
                <div className={styles.bouttons}>
                  <button className={styles.postulerButton} onClick={() => handlepostuler(offre.idOffre, idUser)}>Postuler</button>
